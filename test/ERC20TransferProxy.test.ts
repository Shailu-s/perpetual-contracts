...
describe("ERC20 Transfer Proxy", function(){
  ...
  beforeEach(async()=>{
    ...
    transferProxy = await upgrades.deployProxy(
    TransferProxy,
    [],
    {
  ...
  describe("Transfer From", function(){
    it("Shoud Tranfer tokens using tranfer from method",async()=>{
        ...
        await transferProxy.erc20SafeTransferFrom(USDC.address,account1.address,account2.address,500000);
    ...
    it("Shoud fail to Tranfer tokens using tranfer from method",async()=>{
        ...
        await USDC.connect(account1).approve(transferProxy.address,10000000);
        await transferProxy.addTransferProxyRole(owner.address);
        await expect( transferProxy.connect(account1).erc20SafeTransferFrom(USDC.address,account1.address,account2.address,500000))
