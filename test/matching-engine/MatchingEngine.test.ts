...
describe('MatchingEngine', function () {
  ...
  describe('Cancel orders:', function() {
    ...
    it("will fail to cancel order if maker is not owner", async () => {
      ...
      await expect(
        matchingEngine.cancelOrder(order1)
      ).to.be.revertedWith("not a maker");
    });
    ...
    it("will fail to cancel all orders when salt is too low", async () => {
      await expect(
        matchingEngine.cancelAllOrders(0)
      ).to.be.revertedWith("salt too low");
    });

    it("should cancel all orders", async () => {
      const [owner] = await ethers.getSigners();
      await expect(
        matchingEngine.cancelAllOrders(10)
      ).to.emit(matchingEngine, "CanceledAll").withArgs(owner.address, 10);
    });
  });
  ...
  describe('TransferManager:', function() { 
    it("should set transfer proxy & emit event with proxy address", async () => {
      await expect(
        transferManagerTest.setTransferProxy(erc20TransferProxy.address)
      ).to.emit(transferManagerTest, "ProxyChange").withArgs(erc20TransferProxy.address);
    });
    ...
    it("should set protocol fee & emit event with old & new protocol fee", async () => {
      await expect(
        transferManagerTest.setProtocolFee(100)
      ).to.emit(transferManagerTest, "ProtocolFeeChanged").withArgs(1, 100);
    });
    ...
    it("should call do transfer with fee > 0", async () => {
      ...
      const left = libDeal.DealSide(
        asset,
        erc20TransferProxy.address,
        account1.address
      );

      const right = libDeal.DealSide(
        asset,
        erc20TransferProxy.address,
        account2.address
      );
      ...
      await transferManagerTest.checkDoTransfers(
        left,
        right,
        dealData,
      );
