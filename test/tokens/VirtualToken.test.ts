...
describe("VirtualToken", function () {
  ...
  beforeEach(async () => {
    virtualToken = await upgrades.deployProxy(VirtualToken, ["MyTestToken", "MKT", isBase], {
  ...
  describe("set minter burner role", function () {
    ...
    it("Should fail to set minter role because", async () => {
      await expect(
        virtualToken.connect(account1).setMintBurnRole(account1.address),
  ...
  describe("Mint and burn Tokens", function () {
    ...
    it("should mint maximum tokens", async () => {
      ...
      expect(await virtualToken.balanceOf(account1.address)).to.be.equal(
        "115792089237316195423570985008687907853269984665640564039457584007913129639935",
      );
    });
    it("should fail to mint tokens", async () => {
      await expect(
        virtualToken.connect(account1).mint(account2.address, 100000),
      ).to.be.revertedWith("VirtualToken: Not minter");
    });
